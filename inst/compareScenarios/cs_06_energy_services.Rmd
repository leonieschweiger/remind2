...
indicators_buildings <- c(
  ...
  "UE|Buildings|non-Heating|Electricity|Appliances and Lighting pCap")
indicators_transport <- c(
  ...
  "ES|Transport|Pass|Road|LDV|Three Wheelers pCap",
...
### Time domain
```{r 06_energy_services/Time domain, results = 'asis'}
for (i in indicators[indicators %in% data[["variable"]]]) {
...
```{r 06_energy_services/GDP domain grouped regions}
...
```{r 06_energy_services/GDP domain separated regions, results = 'asis'}
...
## Buildings
### FE mix
```{r 06_energy_services/FE demand from buildings by carriers}
tot <- "FE|Buildings"
items <- c(
  "FE|Buildings|non-Heating|Electricity|Conventional",
  "FE|Buildings|non-Heating|Electricity|Appliances and Lighting",
  "FE|Buildings|non-Heating|Electricity|ICT",
  "FE|Buildings|non-Heating|Electricity|Space Cooling",
  ...
  NULL)
items <- items[items %in% data[["variable"]]]
showAreaAndBarPlots(data, items, tot, orderVars = "user", scales = "fixed")
```

### FE mix per Capita
```{r 06_energy_services/per Capita FE demand from buildings by carriers}
tot <- "FE|Buildings pCap"
items <- c(
  "FE|Buildings|non-Heating|Electricity|Conventional pCap",
  "FE|Buildings|non-Heating|Electricity|Appliances and Lighting pCap",
  "FE|Buildings|non-Heating|Electricity|ICT pCap",
  "FE|Buildings|non-Heating|Electricity|Space Cooling pCap",
  ...
  NULL)
if (any(items %in% data[["variable"]])) {
  showAreaAndBarPlots(data, items, tot, orderVars = "user", scales = "fixed")
}
```

### UE mix
```{r 06_energy_services/UE demand from buildings by carriers}
tot <- "UE|Buildings"
items <- c(
  "UE|Buildings|non-Heating|Electricity|Conventional",
  "UE|Buildings|non-Heating|Electricity|Appliances and Lighting",
  "UE|Buildings|non-Heating|Electricity|ICT",
  "UE|Buildings|non-Heating|Electricity|Space Cooling",
...
### UE mix per Capita
```{r 06_energy_services/per Capita UE demand from buildings by carriers}
tot <- "UE|Buildings pCap"
items <- c(
  "UE|Buildings|non-Heating|Electricity|Conventional pCap",
  "UE|Buildings|non-Heating|Electricity|Appliances and Lighting pCap",
  "UE|Buildings|non-Heating|Electricity|ICT pCap",
  "UE|Buildings|non-Heating|Electricity|Space Cooling pCap",
...
### FE heating
```{r 06_energy_services/FE demand from buildings for heating by carriers}
...
```{r 06_energy_services/per Capita FE demand from buildings for heating by carriers}
...
### UE heating
```{r 06_energy_services/UE demand from buildings for heating by carriers}
...
```{r 06_energy_services/per Capita UE demand from buildings for heating by carriers}
...
#### FE Pass mix by mode w/ bunkers
```{r 06_energy_services/FE Pass mix by mode with bunkers}
...
#### FE Pass mix by mode w/ bunkers - per capita
```{r 06_energy_services/FE Pass mix by mode with bunkers - per capita}
tot <- "FE|Transport|Pass pCap"
items <- c(
  "FE|Transport|Bunkers|Pass|International Aviation pCap",
  "FE|Transport|Pass|Domestic Aviation pCap",
  "FE|Transport|Pass|Rail pCap",
  "FE|Transport|Pass|Road|Bus pCap",
  "FE|Transport|Pass|Road|LDV|Four Wheelers pCap",
  "FE|Transport|Pass|Road|LDV|Three Wheelers pCap",
  "FE|Transport|Pass|Road|LDV|Two Wheelers pCap"
  )
showAreaAndBarPlots(data, items, tot, orderVars = "user", scales = "fixed")
```
...
#### ES Pass mix by mode w/ bunkers
```{r 06_energy_services/ES Pass by mode with bunkers - Bar}
...
#### ES Pass mix by mode w/ bunkers - per capita
```{r 06_energy_services/ES Pass by mode with bunkers - Bar}
tot <- "ES|Transport|Pass pCap"
items <- c(
  "ES|Transport|Bunkers|Pass|International Aviation pCap",
  "ES|Transport|Pass|Domestic Aviation pCap",
  "ES|Transport|Pass|Rail pCap",
...
#### ES Pass share mix by mode w/ bunkers - Bar 
```{r 06_energy_services/ES Pass share mix by mode with bunkers - Bar}
showAreaAndBarPlots(data, items, tot, orderVars = "user", fill = TRUE)
```

#### ES Pass mix by mode w/o bunkers - Bar
```{r 06_energy_services/ES Pass by mode without bunkers - Bar}
...
#### ES Pass share mix by mode w/o bunkers 
```{r 06_energy_services/ES Pass share mix by mode without bunkers}
showAreaAndBarPlots(data, items, tot, orderVars = "user", fill = TRUE)
```

#### ES Pass Modes - Line
```{r 06_energy_services/ES Pass Modes - Line}
showLinePlots(data, "ES|Transport|Pass")
showLinePlots(data, "ES|Transport|Pass|Short-Medium distance")
showLinePlots(data, "ES|Transport|Pass|Road|LDV")
showLinePlots(data, "ES|Transport|Pass|Road|LDV|Four Wheelers")
showLinePlots(data, "ES|Transport|Pass|Road|LDV|Three Wheelers")
...
```{r 06_energy_services/ES Pass LDV by tech - Bar}
...
```{r 06_energy_services/ES Freight by mode - Bar}
...
```{r 06_energy_services/ES Freight with shipping by mode - mode share - Bar}
showAreaAndBarPlots(data, items, tot, orderVars = "user", fill = TRUE)
```

#### ES Freight w/o shipping by mode - Bar
```{r 06_energy_services/ES Freight without shipping by mode - Bar}
items <- c(
  "ES|Transport|Freight|Domestic Shipping",
  "ES|Transport|Freight|Rail",
  "ES|Transport|Freight|Road"
...
```{r 06_energy_services/ES Freight without shipping by mode - mode share - Bar}
showAreaAndBarPlots(data, items, tot, orderVars = "user", fill = TRUE)
```

#### ES Freight Modes - Line
```{r 06_energy_services/ES Freight Modes - Line}
...
```{r 06_energy_services/ES Freight Road by tech - Bar}
...
```{r 06_energy_services/LDV Stock by tech - carrier share - Bar}
showAreaAndBarPlots(data, items, tot, orderVars = "user", fill = TRUE)
```

#### LDV 4Wheelers (Car) Stock by tech - Bar
```{r 06_energy_services/LDV 4Wheelers (Car) Stock by tech - Bar}
tot <- "Stock|Transport|Pass|Road|LDV"
items <- c(
  "Stock|Transport|Pass|Road|LDV|BEV",
  "Stock|Transport|Pass|Road|LDV|Hybrid electric",
...
```{r 06_energy_services/LDV 4Wheelers (Car) Stock by tech - carrier share - Bar}
showAreaAndBarPlots(data, items, tot, orderVars = "user", fill = TRUE)
```

#### LDV Sales by tech - Bar
```{r 06_energy_services/LDV Sales by tech - Bar}
...
```{r 06_energy_services/LDV Sales by tech - carrier share - Bar}
showAreaAndBarPlots(data, items, tot, orderVars = "user", fill = TRUE)
```

#### LDV 4Wheelers (Car) Sales by tech - Bar
```{r 06_energy_services/LDV 4Wheelers (Car) by tech - Bar}
...
```{r 06_energy_services/LDV 4Wheelers (Car) Sales by tech - carrier share - Bar}
showAreaAndBarPlots(data, items, tot, orderVars = "user", fill = TRUE)
```

#### Truck Stock - line
```{r 06_energy_services/Truck Stock - line}
showLinePlots(data, "Stock|Transport|Freight|Road")
```

#### Truck Stock by tech - Bar
```{r 06_energy_services/Truck Stock by tech - Bar}
...
```{r 06_energy_services/Truck Stock by tech - carrier share - Bar}
showAreaAndBarPlots(data, items, tot, orderVars = "user", fill = TRUE)
```

#### Truck Sales - line
```{r 06_energy_services/Truck Sales - line}
showLinePlots(data, "Sales|Transport|Freight|Road")
```

#### Truck Sales by tech - Bar
```{r 06_energy_services/Truck Sales by tech - Bar}
...
```{r 06_energy_services/Truck Sales by tech - carrier share - Bar}
showAreaAndBarPlots(data, items, tot, orderVars = "user", fill = TRUE)
```

#### Bus Stocks - line
```{r 06_energy_services/Bus Stock - line}
showLinePlots(data, "Stock|Transport|Pass|Road|Bus")
```

#### Bus Stock by tech - Bar
```{r 06_energy_services/Bus Stock by tech - Bar}
...
```{r 06_energy_services/Bus Stock by tech - carrier share - Bar}
showAreaAndBarPlots(data, items, tot, orderVars = "user", fill = TRUE)
```

#### Bus Sales - line
```{r 06_energy_services/Bus Sales - line}
showLinePlots(data, "Sales|Transport|Pass|Road|Bus")
```

#### Bus Sales by tech - Bar
```{r 06_energy_services/Bus Sales by tech - Bar}
...
```{r 06_energy_services/Bus Sales by tech - carrier share - Bar}
showAreaAndBarPlots(data, items, tot, orderVars = "user", fill = TRUE)
```

## Industry Production
### Subsector Production
```{r 06_energy_services/plot industry subsector production}
...
```{r 06_energy_services/plot steel production mix, results = 'asis'}
...
```{r 06_energy_services/Steel mix by route}
...
### Steel accumulation
```{r 06_energy_services/plot steel accumulation}
...
```{r 06_energy_services/plot subsector per-GDP production}
