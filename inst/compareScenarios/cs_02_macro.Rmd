...
```{r 02_macro/Capital Stock}
showLinePlots(data, "Capital Stock|Non-ESM")
```

## Investments

```{r 02_macro/Investments}
showLinePlots(data, "Investments|Macroeconomy")
```


## Interest Rate

```{r 02_macro/Interest Rate}
...
```{r 02_macro/Transport Value Disaggregtion}
...
```{r 02_macro/Buildings Value Disaggregtion}

tot <- "Internal|CES Function|Value|enb"
items <- c(
  "Internal|CES Function|Value|enhb",
  "Internal|CES Function|Value|feelcb",
  "Internal|CES Function|Value|feelalb",
  "Internal|CES Function|Value|feelictb",
  "Internal|CES Function|Value|feelscb")
...
```{r 02_macro/Industry Value Disaggregtion}
...
```{r 02_macro/Prices preprocessing}
...
```{r 02_macro/FE Buildings Prices}
...
```{r 02_macro/FE Transport Prices}
carriers <- c("Solids", "Liquids", "Gases", "Hydrogen", "Heat", "Electricity")
...
```{r 02_macro/FE intensity of GDP_PPP, area plot (by sector)}
items <- c(
...
```{r 02_macro/FE intensity of GDP_PPP, line plot (by sector)}
dIea <-
  data %>%
...
```{r 02_macro/Kaya Decomposition Absolute}
showMultiLinePlots(kaya, kayaVars, nrowNum = 2)
walk(kayaVars, showRegiLinePlots, data = kaya)
```

```{r 02_macro/Kaya Decomposition Relative text, results='asis'}
cat("### Relative to ", refYear, "\n")
```

```{r 02_macro/Kaya Decomposition Relative}
