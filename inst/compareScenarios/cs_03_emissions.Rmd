...
```{r 03_emissions/GHG total without Bunkers with national LULUCF accounting - should match UNFCCC}
...
```{r 03_emissions/GHG total with Bunkers with national LULUCF accounting - should match UNFCCC}
showLinePlots(data, "Emi|GHG|w/ Bunkers|LULUCF national accounting")
```

### GHG total w/o Bunkers
```{r 03_emissions/GHG total without Bunkers}
...
### GHG total w/ Bunkers
```{r 03_emissions/GHG total with Bunkers}
showLinePlots(data, "Emi|GHG|w/ Bunkers",
              histModelsExclude = "ClimateTrace")
```

### GHG - total
```{r 03_emissions/GHG total}
...
items <- c(
  "scenario" = "Emi|GHG|w/ Intra-region Bunkers|LULUCF national accounting", # scenario
  "historical" = "Emi|GHG|w/o Bunkers|LULUCF national accounting" # historical
) 
...
### GHG total w/o Bunkers w/o LULUCF
```{r 03_emissions/GHG total without LULUCF}
showLinePlots(data, "Emi|GHG|w/o Bunkers|w/o Land-Use Change", 
              target = "Internal|Emi|GHG|w/o Bunkers|w/o Land-Use Change|NDC Target" )
```
...
```{r 03_emissions/CO2 total without Bunkers with national LULUCF accounting}
...
```{r 03_emissions/CO2 total with Bunkers and national LULUCF accounting}
showLinePlots(data, "Emi|CO2|w/ Bunkers|LULUCF national accounting")
```

### CO2 total w/o Bunkers
```{r 03_emissions/CO2 total without Bunkers}
# TODO: EU sources and PRIMAP still exclude bunkers in default vars -> update
...
### CO2 total w/ Bunkers 
```{r 03_emissions/CO2 total with Bunkers}
showLinePlots(data, "Emi|CO2|w/ Bunkers",
              histModelsExclude = "ClimateTrace")
```

### Total CO2 (for comparison with old REMIND versions)
```{r 03_emissions/Total CO2 (for comparison with old REMIND versions)}
...
```{r 03_emissions/CO2 by sector net incl. BECCS}
tot <- "Emi|CO2"
items <- c(
...
```{r 03_emissions/Energy and Industrial Processes without bunkers - Net (incl BECCS)}
showLinePlots(
  data, 
...
```{r 03_emissions/Energy and Industrial Processes with bunkers - Net (incl BECCS)}
showLinePlots(
  data, 
...
```{r 03_emissions/CO2 Energy and Industry Gross}
showLinePlots(
  data, 
...
```{r 03_emissions/CO2 Energy without bunkers}
showLinePlots(data, "Emi|CO2|w/o Bunkers|Energy")
```

### Energy with bunkers
```{r 03_emissions/CO2 Energy with bunkers}
showLinePlots(data, "Emi|CO2|w/ Bunkers|Energy")
```


### Energy Supply
```{r 03_emissions/CO2 Energy Supply}
showLinePlots(
  data, 
...
### Electricity
```{r 03_emissions/CO2 Electricity}
showLinePlots(
  data, 
...
```{r 03_emissions/CO2 Electricity and Heat}
showLinePlots(data, 
...
### Industry, Gross
```{r 03_emissions/CO2 Industry Gross}
showLinePlots(
  data, 
...
```{r 03_emissions/CO2 Industry with Industrial Processes}
showLinePlots(data, 
...
```{r 03_emissions/Industry Subsector Emissions}
items <- c(
...
### Process Emissions
```{r 03_emissions/CO2 Process Emissions}
showLinePlots(data, "Emi|CO2|Industrial Processes")
```

```{r 03_emissions/CO2 Process Emissions by Sector}
...
```{r 03_emissions/Process Emissions Cement}
showLinePlots(
  data, 
...
```{r 03_emissions/CO2 waste non-incinerated plastics}
...
```{r 03_emissions/CO2 waste non-plastic Materials}

showLinePlots(data, "Emi|CO2|Waste|Non-plastic Waste")
```

### Land-Use Change
```{r 03_emissions/CO2 Land-Use Change}
showLinePlots(
  data, 
...
```{r 03_emissions/CO2 Land-Use Change - LULUCF national accounting}
showLinePlots(
  data, 
...
```{r 03_emissions/novel CDR outside the energy system}
showLinePlots(data, "Emi|CO2|non-ES CDR",
              histModelsExclude = "ClimateTrace")
```

### CDR compared to other CO2 emissions
```{r 03_emissions/CDR compared to other CO2 emissions}
tot <- "Emi|CO2"
items <- c(
  "Emi|CO2|Gross|Energy",
  "Emi|CO2|Gross|Industrial Processes",
  "Emi|CO2|Gross|Waste",
  "Emi|CO2|Land-Use Change",
  "Emi|CO2|CDR|BECCS",
  "Emi|CO2|CDR|Biochar",
...
```{r 03_emissions/CO2 CDR Subcategories - Line}
cdrVars <-
...
## Cumulated Emissions
```{r}
# calculate cumulated CO2 and GHG emissions since 2020 (incl. the 2020 emissions)
dCumul <- data %>%
  filter(grepl("^Emi.*Cumulated", variable))
dCumul <- dCumul %>%
  rename(cum.value = value) %>%
  mutate(variable.noncumul = sub("\\|Cumulated", "", variable)) %>%
  # cum.value2020: cumulated value in 2020
  left_join(
    dCumul %>%
      filter(period == 2020) %>%
      select(model, scenario, region, variable, unit, cum.value2020 = value),
    by = c("model", "scenario", "region", "variable", "unit")) %>%
  # value2020: 2020 emissions of the corresponding non-cumulated variable
  left_join(
    data %>%
      filter(period == 2020) %>%
      mutate(variable.noncumul = as.character(variable)) %>%
      select(model, scenario, region, variable.noncumul, value2020 = value),
    by = c("model", "scenario", "region", "variable.noncumul")) %>%
  mutate(value = (cum.value - cum.value2020 + value2020) / 1000,
         unit = "Gt CO2") %>%
...
pkBudg <- tribble(
      ...
      "World", 2020,    750,    "|target|750 Gt from",
      "World", 2020,    1000,   "|target|1000 Gt from",
  )
```

### CO2 
```{r 03_emissions/CO2 cumulated}
...
### By sector
```{r 03_emissions/CO2 Cumulated bar plots}
tot <- "Emi|CO2|Cumulated"
items <- c(
  "Emi|CO2|Cumulated|Gross|Energy|Supply",
  "Emi|CO2|Cumulated|Gross|Energy|Demand|Buildings",
  "Emi|CO2|Cumulated|Gross|Energy|Demand|Transport",
  "Emi|CO2|Cumulated|Gross|Energy|Demand|Industry",
  "Emi|CO2|Cumulated|Gross|Industrial Processes",
...
### CDR
```{r 03_emissions/CO2 Cumulated Subcategories}
cdrVars <-
...
### GHG ETS Emissions
```{r 03_emissions/GHG ETS Market Emissions}
tot <- "Emi|GHG|ETS"
items <- c(
...
### GHG ESR Emissions
```{r 03_emissions/GHG ESR Market Emissions}
tot <- "Emi|GHG|ESR"
items <- c(
...
```{r 03_emissions/GHG Other Market Emissions}
...
```{r 03_emissions/GHG Market Emissions}
...
### Market CH4
```{r 03_emissions/CH4 Market Emissions}
...
### Market N2O
```{r 03_emissions/N2O Market Emissions}
...
### F-Gases
```{r 03_emissions/F-Gases}
showLinePlots(data, "Emi|GHG|F-Gases")
```

### SO2
```{r 03_emissions/SO2 Emissions}
tot <- "Emi|SO2"
items <- c(
