...
```{r 01_summary/GHG by sector (with gross emissions, excl. negative emissions from CCS of non-fossil carbon)}
tot <- "Emi|GHG"
items <- c(
...
```{r 01_summary/GHG pCap by sector}
showAreaAndBarPlots(data, paste(items, "pCap"), paste(tot, "pCap"), scales = "fixed")
```

## CO2 by sector

```{r 01_summary/CO2 by sector (with gross emissions, excl. negative emissions from CCS of non-fossil carbon)}
tot <- "Emi|CO2"
items <- c(
  "Emi|CO2|Land-Use Change",
...
```{r 01_summary/CO2 pCap by sector}
showAreaAndBarPlots(data, paste(items, "pCap"), paste(tot, "pCap"), scales = "fixed")
```
...
```{r 01_summary/CO2 cumulated by sector (with gross emissions, excl. negative emissions from CCS of non-fossil carbon)}
...
```{r 01_summary/FE per capita (by sector, time domain, area plot)}
items <- c(
  "FE|CDR pCap",
  "FE|Transport pCap",
  "FE|Buildings pCap",
...
```{r 01_summary/FE per capita (by sector, time domain, line graph)}
dIea <- data %>%
...
```{r 01_summary/FE per capita (by sector, GDP)}
dIea <- data %>%
  # To make the plots less crowded, show only IEA historical data.
  filter(scenario != "historical" | model == "IEA")
...
```{r 01_summary/FE by carrier (detailed)}
items <- c(
  "FE|CDR|Electricity",
...
```{r 01_summary/FE Industry by carrier (incl. non-energy use)}
items <- c(
  "FE|Industry|Electricity",
  "FE|Industry|Hydrogen",
  "FE|Industry|Heat",
...
```{r 01_summary/FE Buildings by carrier}
items <- c(
  "FE|Buildings|Electricity",
  "FE|Buildings|Hydrogen",
  "FE|Buildings|Heat",
...
```{r 01_summary/FE Transport by carrier}
items <- c(
  "FE|Transport|Electricity",
  "FE|Transport|Hydrogen",
  "FE|Transport|Gases",
...
```{r 01_summary/FE CDR by carrier}
items <- c(
  "FE|CDR|Electricity",
  "FE|CDR|Hydrogen",
  "FE|CDR|Heat",
...
```{r 01_summary/FE Non-energy Use by carrier}
items <- c(
...
```{r 01_summary/SE Electricity by carrier}
items <- c(
...
```{r 01_summary/CO2 Prices per sector}
items <- c(
...
```{r 01_summary/CO2 Prices ETS-ESR}
...
```{r 01_summary/Tables CO2 Net Zero}
data %>%
  calcTimeSeriesStats(
    "Emi|CO2",
...
```{r 01_summary/Tables GHG Net Zero}
data %>%
  calcTimeSeriesStats(
    "Emi|GHG",
    stats = list("Net 0 Period, i.e., first period with < 2\\% of 2005 value" = function(v, p) {
