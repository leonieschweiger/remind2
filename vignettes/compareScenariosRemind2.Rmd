...
A cs2-profile is a set of arguments for `piamPlotComparison::compareScenarios()`. There are different profiles for a comparison on the 12 main REMIND regions (`H12`, `H12-to2050`), or on the European regions (`EU27`, `EU27-to2050`) for 21-region-runs. The suffix `-to2050` indicates that only periods up to 2050 are considered. The `default`-profiles just call `compareScenarios()` with the default arguments as described in `?piamPlotComparison::compareScenarios`.
...
10. Add a new plot inside the chunk, e.g., `showLinePlots(data, "Your|Variable|Name")`. See [Plot Functions] below. Note:
Colors are handled by the [mip](https://github.com/pik-piam/mip) library. If variable names have identical prefixes or suffixes, these are abbreviated during matching, and only the abbreviated form is used for lookup in the [plotstyles](https://github.com/pik-piam/mip/blob/master/inst/extdata/plotstyle.csv) file.   
